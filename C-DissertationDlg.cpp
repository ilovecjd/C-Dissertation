...
void CCDissertationDlg::OnBnClickedTest()
{
	...
	int* lastResult =new int[iLoop *4 * 4 * 4*printWidth];
		...
		{*/
			...
			for (int h = 0; h < 4; h++){
				for (int m = 0; m < 4; m++){
					...
					CString prarmFile;// = L"d:\\test00.anh";
					prarmFile.Format(_T("D:\\result\\test2_%d%d%d%d.ahn"), i,h,m,l);
					Creator.Save(prarmFile);
						...
						for (int j = 0; j < 50; j++) {
							...
							else
							{
								...
								cashInit += company->m_GlobalEnv.Cash_Init;
						...
						int index = 0;
						*(lastResult+(printRow * printWidth  + index++)) = i;
						*(lastResult+(printRow * printWidth  + index++)) = h;
						*(lastResult+(printRow * printWidth  + index++)) = m;
						*(lastResult+(printRow * printWidth  + index++)) = l;
						*(lastResult+(printRow * printWidth  + index++)) = successCnt;
	...
	pXl->WriteArrayToRange(WS_NUM_DEBUG_INFO, 1, 1, (int*)lastResult, printRow, printWidth); //세로로 출력
